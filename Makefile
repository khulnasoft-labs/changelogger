...
unit: $(RESULTSDIR) fixtures ## Run unit tests (with coverage)
	...
	go test  -coverprofile $(COVER_REPORT) $(shell go list ./... | grep -v khulnasoft-labs/changelogger/test)
...
CHANGELOG.md:
	$(TEMPDIR)/changelogger > CHANGELOG.md
...
release: clean-dist CHANGELOG.md ## Build and publish final binaries and packages.
	...
	# TODO: in the future add changelogger to generate changelogs
